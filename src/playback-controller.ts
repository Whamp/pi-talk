...
export type PlaybackProcess = {
  kill(signal?: NodeJS.Signals): unknown;
  done: Promise<unknown>;
};

export type PlaybackOperations = {
  platform(): NodeJS.Platform;
  commandExists(command: string): Promise<boolean>;
  writeTempAudio(audio: ArrayBuffer): Promise<string>;
  spawn(command: string, args: string[]): PlaybackProcess;
};
...
export function createPlaybackController(options: { config: TalkConfig; ops?: PlaybackOperations }): PlaybackController {
  const ops = options.ops ?? nodePlaybackOperations;
  let active: PlaybackProcess | undefined;
  ...
  async function start(audio: ArrayBuffer, generation: number): Promise<void> {
    const audioPath = await ops.writeTempAudio(audio);
    if (generation !== quietGeneration) return;
    ...
    void active.done.finally(() => {
      ...
      if (next) void start(next, generation);
    });
  }
}

export async function resolvePlaybackCommand(configured: string, ops: Pick<PlaybackOperations, "platform" | "commandExists">): Promise<string> {
  if (configured !== "auto") return configured;
  const candidates = ops.platform() === "darwin" ? ["afplay", "ffplay", "mpv"] : ["pw-play", "paplay", "aplay", "ffplay", "mpv"];
  for (const candidate of candidates) {
    if (await ops.commandExists(candidate)) return candidate;
  }
  throw new Error(`No Playback Command found. Tried: ${candidates.join(", ")}`);
}

function playbackArgs(command: string, audioPath: string): string[] {
  if (command === "ffplay") return ["-nodisp", "-autoexit", "-loglevel", "error", audioPath];
  if (command === "mpv") return ["--no-video", "--really-quiet", audioPath];
  return [audioPath];
}

export const nodePlaybackOperations: PlaybackOperations = {
